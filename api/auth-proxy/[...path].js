...
const DEFAULT_AUTH_API_BASE_URL =
  "http://go-auth-service-alb-192557461.ap-southeast-1.elb.amazonaws.com";
...
function getProxyPath(request) {
  ...
  const prefixes = ["/api/auth-proxy", "/auth-api"];
...
function getTargetUrl(request) {
  const incomingUrl = new URL(request.url ?? "/", "http://vercel.local");
  const targetUrl = new URL(`${getAuthApiBaseUrl()}${getProxyPath(request)}`);
...
function buildForwardHeaders(requestHeaders) {
  ...
  headers.set("x-forwarded-by", "vercel-auth-proxy");
...
export default async function handler(request, response) {
  ...
  try {
    ...
    sendJson(response, 502, {
      error: "Auth service proxy request failed.",
